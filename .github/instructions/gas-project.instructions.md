...
## Банк знаний (Memory Bank)

Проект содержит живую систему документации в `Artifacts/memory-bank/`. ИИ **обязан** обновлять банк знаний после каждого успешного внедрения функционала.

### Файлы банка знаний

| Файл | Назначение | Когда обновлять |
|------|-----------|-----------------|
| `FSD.md` | Функциональные спецификации всех модулей, API, схемы данных | При добавлении/изменении модуля, API, схемы БД |
| `productContext.md` | Бизнес-контекст: какую задачу решает программа | При изменении бизнес-логики или целевой аудитории |
| `activeContext.md` | Текущий фокус работы, последние изменения, известные проблемы | **После каждой сессии работы** — обновлять текущие задачи и недавние изменения |
| `systemArchitecture.md` | Архитектура: модули, взаимодействия, БД, кэш, деплой | При изменении архитектуры, добавлении листов/модулей |

### Правила обновления

1. **После каждого успешного внедрения** функционала — обновить `FSD.md` (спецификация), `systemArchitecture.md` (если менялась архитектура), `activeContext.md` (что сделано).
2. **В начале новой задачи** — прочитать `activeContext.md` для восстановления контекста.
3. **При добавлении нового листа/модуля** — обновить все 4 файла.
4. Добавлять запись в **Changelog** в конце каждого обновлённого файла.
5. Документация пишется **на русском языке**.

### Дополнительные документы

Папка `Artifacts/Documents/` содержит детальные спецификации:
- `FSD MDM Интеграция.md` — подробная спецификация MDM-модуля (EAV, формулы, API-контракты).
