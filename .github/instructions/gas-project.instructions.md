...
| `Change_Logs.md` | Журнал **всех** изменений проекта | **При КАЖДОМ изменении**, даже незначительном |

### Журнал изменений (Change Logs)

Файл `Artifacts/memory-bank/Change_Logs.md` — **обязательный журнал всех изменений**.

**При КАЖДОМ изменении** (даже самом незначительном — правка одной строки, переименование, рефакторинг) **обязательно** добавлять новую строку в таблицу:

| Колонка | Что писать |
|---------|-----------|
| Дата | Дата изменения (YYYY-MM-DD) |
| Описание | Краткое описание что сделано |
| Анализ | Почему потребовалось изменение |
| To Be | Целевое состояние (как должно быть) |
| As Is | Состояние до изменения |
| Что изменено | Конкретные правки (добавлено/удалено/изменено) |
| Файлы | Каждый файл с **новой строки**: `- файл1<br>- файл2<br>- файл3`. Использовать `<br>` для переноса строк внутри ячейки таблицы, каждый файл с префиксом `- ` |

### Правила обновления

1. **После КАЖДОГО изменения** — добавить запись в `Change_Logs.md`. Это **первое действие** после внесения правок.
2. **После каждого успешного внедрения** функционала — обновить `FSD.md` (спецификация), `systemArchitecture.md` (если менялась архитектура), `activeContext.md` (что сделано).
3. **В начале новой задачи** — прочитать `activeContext.md` для восстановления контекста.
4. **При добавлении нового листа/модуля** — обновить все 4 файла.
5. Добавлять запись в **Changelog** в конце каждого обновлённого файла.
6. Документация пишется **на русском языке**.
