...
var SCHEMA = {
  ...
  'Продажи': {
    headers: ['id','purchase_id','qty','buyer','wa','sale_date','manager_id','wallet_id','total_kgs','paid_kgs','debt_kgs','note','shift_id','receipt_id','is_returned','created_at','updated_at'],
    color: '#FCE4EC', tabColor: '#C62828',
    widths: [50,90,60,220,160,110,80,80,120,120,120,200,80,100,80,140,140],
    note: 'debt_kgs обновляется при каждой оплате. qty — ко-во проданного товара',
  },
  'Оплаты': {
...
function _seedData(ss) {
  ...
  _seedSheet(ss, 'Склады', [
    ['id','name','address','responsible','note','current_items','current_cost_kgs','created_at','updated_at'],
  ...
  _seedSheet(ss, 'Закупки', [
    ['id','purchase_date','wh_id','supplier_id','cost_usd','rate','cost_kgs','has_imei','imei','qty','condition','class_id','type_id','product_id','status','note','created_at','updated_at'],
  ...
  _seedSheet(ss, 'Продажи', [
    ['id','purchase_id','qty','buyer','wa','sale_date','manager_id','wallet_id','total_kgs','paid_kgs','debt_kgs','note','shift_id','receipt_id','is_returned','created_at','updated_at'],
    [1,1,1,'Асель Токтогулова','+996700001001',m3ago,1,1,145500,145500,0,'','','SEED-001','FALSE',m3ago,''],
    [2,2,1,'Бакыт Осмонов','+996555002002',m2ago,2,1,128000,80000,48000,'Рассрочка','','SEED-002','FALSE',m2ago,''],
    [3,6,1,'Данияр Иманов','',m2ago,3,3,195000,195000,0,'','','SEED-003','FALSE',m2ago,''],
    [4,10,1,'Айгуль Мамытова','+996700004004',m1ago,1,1,52000,52000,0,'','','SEED-004','FALSE',m1ago,''],
  ]);
  ...
  _seedSheet(ss, 'КассовыеОперации', [
    ['id','op_date','op_type','wallet_id','cat_id','article_id','amount','counterpart','comment','shift_id','created_at','updated_at'],
    [1,m3ago,'Приход',1,1,1,145500,'Асель Токтогулова','Продажа #1',null,m3ago,''],
    [2,m2ago,'Приход',1,1,1,80000,'Бакыт Осмонов','Продажа #2',null,m2ago,''],
    [3,m2ago,'Приход',3,1,1,195000,'Данияр Иманов','Продажа #3',null,m2ago,''],
    [4,m1ago,'Приход',1,1,1,52000,'Айгуль Мамытова','Продажа #4',null,m1ago,''],
    [5,m3ago,'Расход',2,3,4,37840,'Ислам Сатуу','Закупка #1',null,m3ago,''],
    [6,m2ago,'Расход',2,3,4,32560,'Ислам Сатуу','Закупка #2',null,m2ago,''],
    [7,m2ago,'Расход',1,3,5,48400,'Эрлис Трейд','Закупка #6',null,m2ago,''],
    [8,m1ago,'Расход',2,3,6,12320,'Эрлис Трейд','Закупка #10',null,m1ago,''],
    [9,m2ago,'Расход',1,4,8,35000,'','Аренда точки',null,m2ago,''],
    [10,m1ago,'Расход',1,4,8,35000,'','Аренда точки',null,m1ago,''],
    [11,m0,'Расход',1,4,8,35000,'','Аренда точки',null,m0,''],
    [12,m2ago,'Расход',1,5,10,18000,'Айдай Бекова','Зарплата',null,m2ago,''],
    [13,m2ago,'Расход',1,5,10,15000,'Нурлан Осмонов','Зарплата',null,m2ago,''],
    [14,m1ago,'Расход',1,5,10,18000,'Айдай Бекова','Зарплата',null,m1ago,''],
    [15,m1ago,'Расход',1,5,10,15000,'Нурлан Осмонов','Зарплата',null,m1ago,''],
    [16,m1ago,'Расход',1,7,14,8000,'Instagram','Таргет',null,m1ago,''],
    [17,m0,'Расход',1,7,15,5000,'OLX','Продвижение',null,m0,''],
    [18,m0,'Расход',1,8,16,3500,'','Хозтовары',null,m0,''],
  ]);
...
function _seedSheet(ss, name, data) {
  ...
  // Берем актуальные заголовки из SCHEMA
  var headers = SCHEMA[name] ? SCHEMA[name].headers : data[0];
  
  sh.clearContents();
  sh.appendRow(headers);
  
  // Добавляем данные (пропуская первую строку, если это заголовки)
  var rowsToAppend = data[0][0] === 'id' ? data.slice(1) : data;
  rowsToAppend.forEach(function(row) {
    // Если в ряду данных меньше колонок, чем в заголовках, дополняем пустыми значениями
    while (row.length < headers.length) row.push('');
    // Если больше — обрезаем (на всякий случай)
    if (row.length > headers.length) row = row.slice(0, headers.length);
    sh.appendRow(row);
  });
  
  Logger.log('Залито ' + rowsToAppend.length + ' строк → ' + name);
}
